    ...
    async () => {
      try {
        ...
        if (!channel || !channel.isTextBased()) {
          ...
          .setTitle('🗓️ 月初のお知らせ｜成績通知表＆月間モチベ')
          .setColor(0xef4444)
          .setDescription(
            [
              '┃今月も、サークル全体の「成長ログ」を残すために、',
              '┃コンテストの成績と（任意で）月間モチベーションの記録をお願いします。'
            ].join('\n')
          )
          .addFields(
            {
              name: '1️⃣ シーズン成績（コンテスト用）',
              value: [
                '**入力用**',
                '∥ `/moti_input` … 成績入力フォームを開く',
                ...
                '**振り返り用**',
                '∥ `/moti_me` … 直近10件の推移',
                '∥ `/moti_summary` … 直近のシーズン別サマリー',
                '∥ `/moti_summary_all` … 全シーズンのサマリー',
              ].join('\n'),
            },
            {
              name: '2️⃣ 月間モチベ調査（任意参加）',
              value: [
                '※「この1ヶ月でどれくらい育成したか・ファンが増えたか」をざっくり残したい方向けの任意アンケートです。',
                '',
                '**入力用**',
                '∥ `/moti_month_input` … 月間モチベ入力フォームを開く',
                '　・対象月（任意）… `2025-11` など。空欄なら今月として記録',
                '　・今月の育成数 … その月に行った育成回数（増加分）',
                '　・今月のファン数 … その月に増えたファン数',
                '',
                '**振り返り用**',
                '∥ `/moti_month_me` … 自分の月間モチベ推移（直近6ヶ月）',
              ].join('\n'),
            },
          )
