...
## 11) embed（/embed ビルダー）追加

### 追加
- `src/features/embed/*`
  - /embed のビルダー機能（ephemeralパネル + modal入力）
  - カラー指定（指定なし含む）
  - URL付属（メッセージ末尾contentへURL一覧を付属）
  - リンクボタン（ButtonStyle.Link）
  - セレクトメニュー（選択時はephemeral返信：安全デフォルト）

### 変更
- `src/events/interactionCreate.js`
  - roles → embed → moti の順でrouterを呼ぶ
  - 例外でプロセスが落ちないようガード追加

- `src/features/roles/interactionRouter.js`
  - ロール系customIdのみ処理し、それ以外のボタンは未処理として通す（将来拡張前提）
